...
function drawOptionLockPlanningBarDetail() {
  ?>
  <div style="white-space:nowrap; <?php echo (isNewGui())?'margin-right:6px;margin-top:5px;':'position:absolute; bottom:5px;left:10px;'; ?>" class="checkboxLabel">
    <?php if (isNewGui()) {?><?php echo pq_ucfirst(i18n('lockPlanningBarDetail'));?>&nbsp;<?php }?>
    <span title="<?php echo pq_ucfirst(i18n('lockPlanningBarDetail'));?>" dojoType="dijit.form.CheckBox"
      type="checkbox" id="lockPlanningBarDetail" name="lockPlanningBarDetail" class="whiteCheck"
      <?php if ( Parameter::getUserParameter('lockPlanningBarDetail')=='1' or Parameter::getUserParameter('lockPlanningBarDetail')=='') {echo 'checked="checked"'; } ?>  >  
      <script type="dojo/connect" event="onChange" args="evt">
        saveUserParameter('lockPlanningBarDetail',((this.checked)?'1':'0'));
        lockPlanningBarDetail = (this.checked)?'1':'0';
      </script>                    
    </span>
    <?php if (!isNewGui()) {?>&nbsp;<?php echo i18n('lockPlanningBarDetail');?><?php }?>
  </div>
<?php 
}
?>