  ...
  public function save() {
    ...
    if ($old->idProject != $this->idProject) {
      ...
      if ($proj->paused) {
        ...
        $oldProj=new Project($old->idProject);
        if ($oldProj->paused) {
          $this->fixPlanning=0;
          $this->paused=0;
          $type=$this->refType;
...
//       }
  ...
  public function indent($way) {
  	...
  	if ($way=="decrease") {
  	  ...
  	  $prec=null;
  		$precs=$this->getSqlElementsFromCriteria(null,false,"wbsSortable<'".$this->wbsSortable."' and idProject in " . getVisibleProjectsList(true),"wbsSortable desc");
  		if (count($precs)>0) {
  			foreach ($precs as $pp) {
    ...
    if (PlanningElement::$_noDispatch and !$force) return;
  	...
  	foreach ($list as $pe) {
  	  if($pe->refType=='Project' and Project::isTheLeaveProject($pe->refId))continue;
  			...
  			$oldWbs=$pe->wbs;
  			$pe->wbs=($root=='')?$idx:$root.'.'.$idx;
  			if ($pe->wbs!=$oldWbs) debugTraceLog("Change WBS of \t$pe->refType #$pe->refId \tfrom $oldWbs \t to $pe->wbs ");
  			if ($pe->refType) {
  				$pe->wbsSave();
  			}
  	}
