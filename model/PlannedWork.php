...
//       }
    ...
    foreach($listProjectsDelay as $idProj=>$delay) {
      ...
      // TODO : add constraint from predécessors
      // TODO : add contraints from first fixed milestone
    }
...
//       }
      ...
      if (isOffDay($startPlan)) {
        while (isOffDay($startPlan)) {
      ...
      if ($plan->refType=='Milestone') {
        ...
        if ($profile=="FDUR") {
          if (! $plan->realStartDate) {
            if ($plan->elementary) {
              if ($plan->plannedWork==0 or $plan->isManualProgress) $plan->plannedStartDate=$startPlan;
...
// Functions for PLAN
              ...
              if ($pred->refType=='Milestone' and $pred->validatedEndDate and ($pred->_profile=='FIXED' or ($pred->_profile=='FLOAT' and count($pred->_directPredecessorList)==0))) {
                if ($dep->dependencyType=='E-S' or $dep->dependencyType=='S-S') {
                  if ($dep->dependencyDelay>0) {
                    $reserved[$artype][$plan->id]['start']=addWorkDaysToDate($predEnd,$dep->dependencyDelay+1);
                  } else {
                    $reserved[$artype][$plan->id]['start']=addWorkDaysToDate($predEnd,$dep->dependencyDelay);
                  }
                } else if ($dep->dependencyType=='E-E') {
                  if ($dep->dependencyDelay>0) {
                      $reserved[$artype][$plan->id]['end']=addWorkDaysToDate($predEnd,$dep->dependencyDelay+1);
                    } else {
                      $reserved[$artype][$plan->id]['end']=addWorkDaysToDate($predEnd,$dep->dependencyDelay);
                    }
                  }
              }
            }
  ...
  private static function storePlannedWorkLeveled($startPlanDate, $expectedStart, $expectedEnd, $capacity, &$ass, &$plan, &$arrayPlannedWork, &$changedAss, &$left, $delay) {
    ...
    if ($prf=='FULL' or $prf=='HALF' or $prf=='QUART' or $prf=='REGUL' or $prf=='FIXED') {
      ...
      $end=$expectedEnd;
      $start=addWorkDaysToDate($end,1-$plan->validatedDuration);
    } else {
      $end=$expectedEnd;
      $start=addWorkDaysToDate($end,1-$minDuration);
    }
