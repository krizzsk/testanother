...
class Audit extends SqlElement {
	...
	static function updateAudit() {
		...
		if (! $audit->id) {
			...
			$forceUpdate=true;
		} else if ($audit->requestDisconnection or $audit->idle==1) {
			...
			if ($script == 'checkAlertToDisplay.php') {
				...
				Audit::finishSession ();
				exit ();
			} else if ($script == 'checkAlertToDisplayNotification.php') {
			  echo "##!##!##!##!##!##";
    ...
    if ($currentTime-$lastTime<$updateDelay and !$forceUpdate) {
      return;
    }
