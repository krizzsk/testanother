...
class Favorite extends SqlElement {
  ...
  static function drawReportList($idFavoriteRow=null) {
    $list=self::getReportList();
    if (count($list)==0) {
      return;
    }
    echo '<form dojoType="dijit.form.Form" id="favoriteReportsForm_'.$idFavoriteRow.'" name="todayParametersForm" onSubmit="return false;">';
    ...
    foreach($list as $rpt) {
      ...
      foreach ($params as $paramName=>$paramValue) {
        $urlParam.=($urlParam or pq_strpos($rpt['fileName'],'?')>0)?'&':'?';
      ...
      echo '<a '; echo 'onClick="removeFavoriteReport(' . htmlEncode($favorite->id). ', \''.$idFavoriteRow.'\');" />';
      if(isNewGui()){
