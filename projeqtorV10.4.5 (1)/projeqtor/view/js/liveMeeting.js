...
function liveMeetingSave(noHistory){
  ...
  dojo.xhrPost({
    ...
    +"&idSpeaker="+allwhoSpeak+"&allOrganizator="+allOrganizator+"&allCanSpeak="+allCanSpeak+"&idLiveMeeting="+dojo.byId("liveMeetingId").value+"&noHistory="+(noHistory ? 1 : 0)+"&csrfToken="+csrfToken,
...
function liveMeetingResizeEditor(loopCount) {
  ...
  if (editorType=="CK") { // CKeditor type
    ...
    if (editor.status=='ready') {
      ...
      if (dojo.byId("kanbanContainer")) {
        var height=dojo.byId("kanbanContainer").offsetHeight; 
        dojo.query(".kanbanColumn.dojoDndSource.dojoDndTarget").forEach(function(domNode) {
          domNode.style.height=(height-90)+"px";
          domNode.style.maxHeight=(height-90)+"px";
        });
        ...
        dojo.query(".liveMeetingBottomDivs").forEach(function(domNode) {
          domNode.style.height=(height-55)+"px";
          domNode.style.maxHeight=(height-55)+"px";
        });
      }
    } else {
...
function addNewLinkMeeting (item,comboName,idProj){
  ...
  if(objName && objClass){
    ...
    showWait();
    dojo.xhrPost({
          url : "../tool/saveLiveMeetingAttachment.php?objectClass="+objClass+'&objName='+objName+'&idProj='+idProj+"&csrfToken="+csrfToken,
          form :null,
          handleAs : "text",
