    ...
    for (var i = 0; i < items.length; i++) {
     ...
     // END MODIF qCazelles - GANTT
      ...
      g.AddTaskItem(new JSGantt.TaskItem(item.id, pName, pStart, pEnd, pColor, pItemColor,
          ...
          item.isoncriticalpath,pobjecttype,pExtRessource,pDurationContract,elementIdRef,item.iconClass,pColorBlindColor,pColorBlindTaskColor,pResponsible,pRespInitial,pColorBaselineBottom,pColorBaselineUpper));
    }
...
function editObjectFromContextMenu(refId, refType){
  fromContextMenu = true;
  var canCreate = (canCreateArray[refType] == 'YES')?1:0;
...
function copyObjectFromContextMenu(refId, refType){
  fromContextMenu = true;
...
function sendMailFromContextMenu(refId, refType){
  dojo.byId('objectClass').value = refType;
  dojo.byId('objectId').value = refId;
  showMailOptions();
}

function showHistoryFromContextMenu(refId, refType){
  var params="&objectClass=" + refType + "&objectId=" + refId;
  loadDialog('dialogHistory', null, true, params);
}

function successorFromContextMenu(refId, refType){
  dojo.byId('objectClass').value = refType;
  dojo.byId('objectId').value = refId;
  indentTask("increase");
}

function predecessorFromContextMenu(refId, refType){
  dojo.byId('objectClass').value = refType;
  dojo.byId('objectId').value = refId;
  indentTask("decrease");
}